...
for trial in range(trials):
    ...
    x = model.addVars(15, 15, lb=0, vtype=GRB.INTEGER, name="from row to columns")
    ...
    tran_cost = 1.0/samples * gb.quicksum(cost_matrix.iloc[i,j]*x[i,j] for i in range(store) for j in range(store))
    model.setObjective(underage_cost + overage_cost + tran_cost, GRB.MINIMIZE)
    
    # Demand constraint
    for s in range(samples):
        for i in range(store):
            model.addConstr(y[i] + d_minus[i, s] + gb.quicksum(x[k,i] for k in range(store)) == np.random.binomial(n[i], p[i]) + d_plus[i, s] + gb.quicksum(x[i,k] for k in range(store)))
